...
CGIAR_admin1_sf<-sf::read_sf("Data/GADM/ADMIN1.shp")
CGIAR_admin1_sf$ADMIN<-CGIAR_admin1_sf$NAME_1
CGIAR_admin1<-terra::vect(CGIAR_admin1_sf)

CGIAR_countries_sf<-sf::read_sf("Data/Region/CGIAR_countries_simplified.shp",options = "ENCODING=UTF8")
...
The level of spatial aggregation determines if statistics are calculated at 1) CG region; 2) Country; or 3) GADM admin 1 levels. At the country level all countries within selected CG regions are analyzed. If you have chosen the `Country` level you can select which CG regions to include in analysis (default = all CG regions). If you have chosen the `Admin1` level, you can select a focal country. Whilst we suggest you analyze these sub-national areas a single country at a time, it is possible to select multiple countries.
...
```{r choose region, echo=F, warning=F}
  ...
  checkboxGroupInput(inputId="Region", 
             ...
             choices = sort(unique(CGIAR_countries2$CG_REG)), 
             selected = unique(CGIAR_countries2$CG_REG),
             inline = T,
             width="400px")
```
:::
...
ExtractBy<-reactive({
  if(input$Analysis.Level=="Region"){
    ...
    CGIAR_countries2[CGIAR_countries2$CG_REG %in% input$Region,]
    }else{
...
ExtractBySf<-reactive({
  if(input$Analysis.Level=="Region"){
    ...
    if(input$Analysis.Level=="Country"){
    CGIAR_countries2_sf[CGIAR_countries2_sf$CG_REG %in% input$Region,]
    }else{
...
Analysis.Level<-reactive({
  if(input$Analysis.Level=="Region"){
     CGIAR_regions$CG_REG
  }else{
    if(input$Analysis.Level=="Country"){
    CGIAR_countries2$ADMIN[CGIAR_countries2$CG_REG %in% input$Region]
    }else{
...
#### Geographies
Load CGIAR countries and regions:
```{r CGIAR_countries,echo =T,eval=F}
CGIAR_countries <- terra::vect("Data/Region/CGIAR_countries_simplified.shp")
```

Create CGIAR regions layer:
```{r Create CGIAR regional layer}
CGIAR_regions<-terra::aggregate(CGIAR_countries2,by="CG_REG",dissolve=T)
...
CGIAR_admin1<- terra::vect("Data/GADM/ADMIN1.simple.shp")
```

```{r plot CGIAR Regions, echo =F}
renderPlot({terra::plot(CGIAR_regions,
                        "CG_REG",
                        legend=T,
                        main="CG Regions",
                        ...
                        type="classes",
                        col=sample(viridis(n=length(CGIAR_regions),option =input$Palette),replace=F),
...
Convert WDPA data to raster format (note that a pre-processed dataset is provided for this markdown):
```{r how to process WDPA_rast,eval=F}
  WDPA_Rast_Name<-"Data/WDPA/WDPA_CGIAR_countries.tif"
...
GPLS_Legend<-fread("Data/GLPS/LPS_legend_RGB.csv")
...
PovClassFun2<-function(Data,THigh,TMed,TLow,Name){
  ...
  levels(Data)<-c("low_medium","medium_high","high")
  names(Data)<-Name
  return(Data)
}
...
ClassifyHazards<-function(Data,Threshold,Mask=NULL){
  ...
  X<-unlist(lapply(str_locate_all(str_pad(values, length(labels), pad = "0"),"1"),FUN=function(X){paste(rev(labels[unlist(X[,1])]),collapse="+")}))
...
renderPlot({
  terra::plot(Hazards_Class2()$Complete,
       col=viridis(n=length(levels(Hazards_Class2()$Complete)[[1]]),option = input$Palette,direction = 1),
       maxcell=input$MaxPixels,
       pax=list(cex.axis=input$TextSize),
       plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
       cex.main=input$TextSize,
...
renderPlot({
  terra::plot(Hazards_Class2_Masked()$Complete,
       col=viridis(n=length(levels(Hazards_Class2_Masked()$Complete)[[1]]),option = input$Palette,direction = 1),
       maxcell=input$MaxPixels,
       pax=list(cex.axis=input$TextSize),
       plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
       cex.main=input$TextSize,
...
renderPlot({
  terra::plot(Hazards_Class2_Masked2(),
       col=viridis(n=length(levels(Hazards_Class2_Masked2())[[1]]),option = input$Palette,direction = 1),
       maxcell=input$MaxPixels,
       pax=list(cex.axis=input$TextSize),
       plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
       cex.main=input$TextSize,
...
if(Fill=="Hazard"){
  if(Data[,length(unique(ADMIN))]>MaxRows){
    ...
    X[,OrderVar:=Data[,..OrderBy]][,list(OrderVar,ADMIN)]
    Keep<-X[,list(Total=sum(OrderVar)),by=ADMIN][order(Total,decreasing = T)][1:MaxRows,ADMIN]
    Data<-Data[ADMIN %in% Keep]
  }
  }else{
    if(Data[,length(unique(Hazard_Class))]>MaxRows){
            X<-data.table::copy(Data)
            X[,OrderVar:=Data[,..OrderBy]][,list(OrderVar,Hazard_Class)]
            Keep<-X[,list(Total=sum(OrderVar)),by=Hazard_Class][order(Total,decreasing = T)][1:MaxRows,Hazard_Class]
...
RHE_Plotter<-function(Data,Fill,TextSize,Palette,LegPos,OrderBy,Alpha,MaxRows,CoordFlip,AreaWeight,FontFace,LegCols,PanelSpace){
      ...
      if(Fill=="Hazard"){
          if(Data[,length(unique(LPS))]>MaxRows){
            X<-data.table::copy(Data)
            X[,OrderVar:=Data[,..OrderBy]][,list(OrderVar,LPS)]
            Keep<-X[,list(Total=sum(OrderVar)),by=LPS][order(Total,decreasing = T)][1:MaxRows,LPS]
  ...
  Data<-melt(Data,id.vars=c("Hazard_Class","LPS"))
    ...
    if(Fill=="Hazard"){
        Order<-Data[variable==OrderBy][,list(value=sum(value)),by=LPS][order(value,decreasing = F),LPS]
        Data[,LPS:=factor(LPS,levels=Order)]
    }else{
      Order<-Data[variable==OrderBy][,list(value=sum(value)),by=Hazard_Class][order(value,decreasing = F),Hazard_Class]
      Data[,Hazard_Class:=factor(Hazard_Class,levels=Order)]
    }
...
UpdateFAOCountries<-data.table(
  FAO=c("Bolivia (Plurinational State of)","Iran (Islamic Republic of)","Syrian Arab Republic","Congo","Viet Nam","Lao People's Democratic Republic","Côte d'Ivoire","Timor-Leste","Brunei Darussalam","Eswatini","Venezuela (Bolivarian Republic of)","Bahamas"),
  CGIAR=c("Bolivia","Iran","Syria","Republic of the Congo","Vietnam","Laos","Ivory Coast","East Timor","Brunei","eSwatini","Venezuela","The Bahamas")
  )
...
renderPlot({terra::plot(mask(c(FAO.CH4.Change,FAO.N2O.Change),ExtractBy()),
                        main="Proportional increase in livestock emissions 2000-2019 (FAOSTAT)",
                        pax=list(cex.axis=input$TextSize),
                        plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
                        col=viridis(n=input$Ncolours,option =input$Palette,direction = if(input$Palette=="turbo"){1}else{-1}),
                        cex.main = input$TextSize*1.2,
...
Update Herrero 2000 data using FAO change between 2000-2019:
...
```
*Note units are CO2eq kg/km2/yr*

Update Herrero 2000 data using FAO change btw 2000-2019:

```{r Plot emissions data, echo =F}
renderPlot({terra::plot(mask(LS.CO2e(),ExtractBy()),
                        pax=list(cex.axis=input$TextSize),
                        plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
                        col=viridis(n=input$Ncolours,option =input$Palette,direction = if(input$Palette=="turbo"){1}else{-1}),
                        cex.main = input$TextSize*1.2,
...
Calculate mean CarbonStock per forest cell (MtC/ha) in each WRI polygon then add to WRI polygons:
...
Calculate Forest Carbon loss per WRI polygon (MtC/ha/yr):
...
renderPlot({terra::plot(mask(WRI.Pasture.CarbonLoss.ha.yr,ExtractBy()),
                        main="CO2e Mt/ha emissions from forest loss due to pasture",
                        pax=list(cex.axis=input$TextSize),
                        plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
                        col=viridis(n=input$Ncolours,option =input$Palette,direction = if(input$Palette=="turbo"){1}else{-1}),
                        cex.main= input$TextSize*1.2,
...
renderPlot({terra::plot(mask(WRI.Soy.CarbonLoss.ha.yr,ExtractBy()),
                        main="CO2e Mt/ha emissions from forest loss due to soy",
                        pax=list(cex.axis=input$TextSize),
                        plg=list(x=input$LegPos,cex = input$TextSize,ncol=input$LegCols),
...
```{r Plot combined WRI commodity loss per ha, echo =F}
...
Load  [Trends.Earth data](https://trends.earth/docs/en/background/understanding_indicators15.html#soil-organic-carbon) soil organic carbon percent change (2001-2015) data:
*Units are %*
```{r Load Trends.Earth SOC %, eval=T}
SOC.pch<-terra::resample(terra::rast("Data/TrendsEarth/soc_pch5000.tif"),BaseRaster)
```

```{r mask Trends.Earth SOC pch to LPS not in protected areas, eval=T}
SOC.pch<-terra::mask(terra::mask(SOC.pch,LPS_Suitable),WDPA_rast,inverse=T)
```

```{r Plot Trends.Earth SOC pch Data, echo =F, out.width="100%", fig.asp = .5}
cuts <- unique(quantile(SOC.pch[],probs=seq(0,1,0.1),na.rm=T))

renderPlot({terra::plot(mask(SOC.pch,CGIAR_countries2),
...
ExtractByLSS<-function(ForestLoss,DirectEmission,Livestock,ExtractBy,LPS){
  ...
  setnames(C,c("area","OID"),c("Area","ADMIN"))
...
SOC.pch.CellSize<-cellSize(SOC.pch,unit="km")
...
SOC_ExtractByLPS<-function(LPS,SOC.pch,ExtractBy,CellSize.km,MinT){
  ...
  setnames(A,c("OID","area"),c("LPS","soc_pch.neg.area"))
...
4) `GHG Emissions & Soil Degradation/Results/Emissions/Livestock Systems (Bars)`   
5) `GHG Emissions & Soil Degradation/Results/SOC in LPS/Geography (Bars)`   
6) `GHG Emissions & Soil Degradation/Results/SOC in LPS/Livestock Systems (Bars)`  
...
CombineData2<-function(CO2e,SOC,Exposure){
  ...
  if(!is.null(Exposure)){
    ...
    setnames(Exposure,"Hazard_Class","fill")
      ...
      Exposure[variable=="Total value of production ($ billion)",variable.code:="V"]
      Exposure[variable=="Total pasture area (million ha)",variable.code:="P"]
      Exposure[variable=="Total rural population (million)",variable.code:="R"]
      Exposure[variable=="Total TLU (million)",variable.code:="L"]
    
      Exposure[variable=="Total value of production ($ billion)",variable:="Value"]
      Exposure[variable=="Total pasture area (million ha)",variable:="Pasture"]
      Exposure[variable=="Total rural population (million)",variable:="Population"]
      Exposure[variable=="Total TLU (million)",variable:="LUs"]
...
```{r Combined - Plot Palette, echo=F}
  selectInput(inputId="Combined.Palette", 
               label= "Plot palette", 
               choices = c("Brown", "DkBlue", "DkCyan", "DkViolet", "GrPink"),
               selected = "Brown",
               multiple = F,
               width="200px")
...
CombinedMapFun<-function(ExtractBy,Data,Dim,TextSize,Xlab,Ylab,LegendSize,LegXPos,LegYPos,Palette,Colour,BreakType,LegCols){
  ...
  map <- ggplot() +
    ...
    biscale::bi_scale_fill(pal=Palette, dim=3) +
    biscale::bi_theme()+
...
```{r Combined - plotly bars,echo=F,out.width="70%"}
  renderPlotly({
      ggplotly(CombinedPlot()$plot)
  })
```
